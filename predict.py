...
class NextSentencePredict:
    def __init__(self, ):
        """初始化EAST"""
        graph_bert = tf.Graph()
        self.sess_bert = tf.Session(graph=graph_bert, config=sess_config)
        with gfile.FastGFile(cfg.fine_tuning_pb_model_dir, 'rb') as f:
            graph_bert_def = tf.GraphDef()
            graph_bert_def.ParseFromString(f.read())

            # trt_east_graph = trt.create_inference_graph(
            #     input_graph_def=graph_bert_def,
        ...
        with graph_bert.as_default():
            tf.import_graph_def(graph_bert_def, name='fine-tuned-bert_1')  # 导入计算图

        self.input_ids = self.sess_bert.graph.get_tensor_by_name('fine-tuned-bert_1/input_ids:0')
        self.input_mask = self.sess_bert.graph.get_tensor_by_name('fine-tuned-bert_1/input_mask:0')
        self.segment_ids = self.sess_bert.graph.get_tensor_by_name('fine-tuned-bert_1/segment_ids:0')

        self.operation_ = self.sess_bert.graph.get_tensor_by_name('fine-tuned-bert_1/log_prob:0')

    # @timed
    def model_predict(self, id, mask, seg_id):
        feed_dict = {self.input_ids: id, self.input_mask: mask, self.segment_ids: seg_id}
        output_ = (self.sess_bert.run(self.operation_, feed_dict))
...
def next_sentence_predict(test, next_sentence_predictor):
    ...
    log_pro = next_sentence_predictor.model_predict(id, mask, seg_id)
...
next_sentence_predictor = NextSentencePredict()
# input_ids, input_mask, segment_ids, operation_, sess = get_model_graph(cfg.fine_tuning_pb_model_dir)
predicted_labels = next_sentence_predict(test, next_sentence_predictor)
